...
export default function Appointment() {
  ...
  return (
    <div className="mt-[64px] min-h-screen bg-gradient-to-b from-white to-gray-50 text-slate-800 font-sans">
      ...
      {/* Header Section */}
      <div className="relative py-20 px-6 bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 text-white text-center overflow-hidden">
        <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-playfair font-bold mb-6 animate-fade-in-up">
            L'Art de la Coiffure Afro
          </h1>
          <p className="text-lg md:text-xl font-light text-purple-100 max-w-2xl mx-auto leading-relaxed">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        {/* Carousel Section */}
        <div className="mb-20">
          <h2 className="text-3xl font-playfair font-bold text-center mb-12 text-gray-900 border-b-2 border-purple-100 pb-4 inline-block mx-auto">
            Nos Prestations Exclusives
          </h2>

          {servicesByCategory.map(([category, items]) => (
            <div key={category} className="mb-16">
              <h3 className="text-xl font-semibold mb-6 px-2 text-purple-900 uppercase tracking-widest text-center md:text-left">{category}</h3>
                  ...
                  <div key={srv.id} className="px-3 py-2 h-full">
                    ...
                    >
                      <div className="relative h-64 overflow-hidden group">
                        ...
                        <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm px-4 py-1 rounded-full text-purple-900 font-bold shadow-sm">
                          {srv.price} €
                        </div>
                      </div>
                      <div className="p-6 flex-1 flex flex-col justify-between text-center">
                        <h4 className="text-lg font-bold text-gray-800 mb-2 font-playfair">{srv.name}</h4>
                        <button className="text-sm font-medium text-purple-700 hover:text-purple-900 mt-2 uppercase tracking-wider">
                          Choisir ce soin
                        </button>
                      </div>
                    </div>
        ...
        <div id="calendar-section" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start scroll-mt-24">
          ...
          <div className="lg:col-span-7 space-y-8 animate-fade-in">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-700 py-4 px-6 pr-8 rounded-xl leading-tight focus:outline-none focus:bg-white focus:border-purple-500 transition-colors cursor-pointer text-base font-medium"
                >
            ...
            {/* Calendar Container */}
            {selectedService && (
              <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 transition-all duration-500">
                <h2 className="text-lg font-playfair font-bold mb-6 text-purple-900 flex items-center gap-2">
                ...
                <Calendar
                  ...
                  tileClassName="h-14 flex items-center justify-center relative font-medium text-sm md:text-base"
                />
          ...
          <div className={`lg:col-span-5 transition-all duration-500 ${selectedSlot ? 'opacity-100 translate-x-0' : 'opacity-50 translate-x-4 grayscale pointer-events-none lg:sticky lg:top-28'}`}>
            <div className="bg-white p-8 rounded-2xl shadow-2xl border-t-4 border-purple-900 border-opacity-100">
              <h2 className="text-2xl font-playfair font-bold mb-8 text-gray-900">
                Votre Réservation
              </h2>
              ...
              {selectedSlot ? (
                <div className="space-y-6">
                  <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                    <p className="text-sm text-purple-900 font-semibold uppercase tracking-wider mb-2">Créneau sélectionné</p>
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-bold text-gray-900 capitalize">{formatDateFr(new Date(selectedSlot.startAt))}</span>
                    </div>
                ...
                <div className="py-12 flex flex-col items-center justify-center text-center text-gray-400">
                  ...
                  <p className="text-lg font-medium">Sélectionnez une date et un créneau à gauche pour continuer.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {showSlotModal && selectedDate && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden transform transition-all scale-100">
              <div className="bg-purple-900 p-6 flex justify-between items-center text-white">
                  ...
                  <div className="space-y-3">
                    {modalSlots.map((sl) => (
                      <button
                        ...
                        className={`w-full group flex items-center justify-between p-4 rounded-xl border transition-all duration-200 ${selectedSlot?.id === sl.id ? 'border-purple-600 bg-purple-50 ring-1 ring-purple-600' : 'border-gray-100 hover:border-purple-300 hover:bg-gray-50'}`}
                      >
                        <div className="text-left">
                          <div className="flex items-center gap-2">
                            <Clock className="w-4 h-4 text-purple-600" />
                            <span className="font-bold text-gray-800">{timeFR(sl.startAt)}</span>
                          </div>
                          <span className="text-xs text-gray-500 block mt-1">Fin à {timeFR(sl.endAt)}</span>
                        </div>
                        <div className="bg-white text-purple-900 text-sm font-semibold px-4 py-2 rounded-lg shadow-sm group-hover:bg-purple-600 group-hover:text-white transition-colors">
                          Réserver
                        </div>
                      </button>
                    ))}
